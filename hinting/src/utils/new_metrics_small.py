...
def score_prob(cands, refs, types, eval_sents=None, mask_rel=True, kg_type='atomic',models=None,tokenizers=None,indexes=None):
    inpt_idx = indexes.get(timeout=5)
    model = models[inpt_idx]
    tokenizer = tokenizers[inpt_idx]
    ...
    steps = 0
    batch_size = 2
    inputs = inputs.long().to(model.device)
    input_mask = mask.to(model.device)
