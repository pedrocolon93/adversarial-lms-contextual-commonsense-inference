...
def beam_search(model, encoder, XMB, start_id, num_beams=1, max_length=gen_len, temperature=1, length_penalty=1,mem=None,size_mem=0,use_mem=False):
    ...
    """
    pad_token_id = encoder.encode('<|PAD|>')[0]
    eos_token_ids = [encoder.encode('<|endoftext|>')[0]]
    ...
    while step < max_length:
        ...
        # beam_scores = beam_scores.to("cpu")
        # scores = scores.to("cpu")
        scores = scores.to(beam_scores.device)
...
def beam_search_bart(model, encoder, XMB, start_id, num_beams=1, max_length=gen_len, temperature=1, length_penalty=1,
                mem=None, size_mem=0, use_mem=False):
    """ Generate sequences for each example with beam search.
    """
    pad_token_id = encoder.encoder['<pad>']
    eos_token_ids = [encoder.encoder['</s>']]
    # generated hypotheses
    vocab_size = len(encoder)
    generated_hyps = [
        BeamHypotheses(num_beams, max_length, length_penalty, early_stopping=False) for _ in range(1)
