    ...
    for idx in iter_data2(*shuffle(idxs, random_state=np.random),
        ...
        for row in range(batch_size):
            ...
            if hint == 1 and args.use_hint:
                ...
                stry = [text_encoder.bos_token_id]+\
                       text_encoder.convert_tokens_to_ids(text_encoder.tokenize(story)) + special + hint_enc  + [text_encoder.eos_token_id]
                ...
                tgt = [text_encoder.bos_token_id]+text_encoder.convert_tokens_to_ids(text_encoder.tokenize(obj))
                ...
                with text_encoder.as_target_tokenizer():
                    final_tgt = text_encoder(obj,max_length=max_target_length,padding="max_length")['input_ids']
                    final_tgt = [-100 if t_ == text_encoder.pad_token_id else t_ for t_ in final_tgt]
                ...
                stry = [text_encoder.bos_token_id] + text_encoder.convert_tokens_to_ids(
                    text_encoder.tokenize(story)) + special + [text_encoder.eos_token_id]
                att_mask = [1 for i in range(len(stry))] + [0 for i in range(max_source_length - len(stry))]
