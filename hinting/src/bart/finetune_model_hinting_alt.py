...
from utils import (encode_dataset3, iter_data2, ResultLogger, make_path)
...
def transform_story(X1, X2):
    ...
    xmb_kg = np.tile(np.array([text_encoder.encoder['<|PAD|>']] * n_ctx,dtype=np.int32),(n_batch,1))
...
def pad_rels(relation, pad_len=100):
    return relation[:100] + [text_encoder.encoder['<|PAD|>']] * (100-len(relation[:100]))
    ...
    for idx in iter_data2(*shuffle(idxs, random_state=np.random),
        ...
        for row in range(batch_size):
            ...
            with text_encoder.as_target_tokenizer():
                dec_tgt = text_encoder.decode([x if x!=-100 else text_encoder.pad_token_id for x in model_inputs["labels"][row,:].tolist()],skip_special_tokens=True).replace("<s>","").replace("</s>","").replace("<pad>","")
            ...
            if hint == 1:
                ...
                x = src
                special = text_encoder.encode("<|sent" + str(idx_of_period) + "|>",add_special_tokens=False)

                stry = [text_encoder.encoder["<s>"]]+text_encoder.convert_tokens_to_ids(text_encoder.tokenize(story)) + hint_enc + special + [text_encoder.encoder["</s>"]]
                att_mask = [1 for i in range(len(stry))]+[0 for i in range(max_source_length-len(stry))]
                stry += [text_encoder.encoder["<pad>"] for i in range(max_source_length-len(stry))]
                tgt = [text_encoder.encoder["<s>"]]+[*text_encoder.encode(dim,add_special_tokens=False), *text_encoder.encode(" , ",add_special_tokens=False)]+text_encoder.convert_tokens_to_ids(text_encoder.tokenize(obj))
                # with text_encoder.as_target_tokenizer():
                #     test = text_encoder(obj)

                additionals = [text_encoder.encoder["</s>"]]
...
if __name__ == '__main__':
    ...
    try:
        trX_kg, trX_st, trMem, trIds, vaX_kg, vaX_st, vaMem, vaIds =  pickle.load(open(data_dir + '/' + 't_' + 'c' * args.comet + 'h' * (1-args.comet) + '_' + args.use_filter * 'filtered_' + args.kg_type + '_' + 'databart.pkl','rb'))
    except:
        try:
            data_dump = pickle.load(open(data_dir + '/' + 'c' * args.comet + 'h' * (1-args.comet) + '_' + args.use_filter * 'filtered_' + args.kg_type + '_' + 'databart.pkl','rb'))
            ...
            ((trX1, trX2, trMem, trIds),
             (vaX1, vaX2, vaMem, vaIds)) = encode_dataset3(*STORIES,encoder=text_encoder,max_source_length = max_source_length, max_target_length = max_target_length)
            print("Dumping...")
            pickle.dump([(trX1,trX2, trMem, trIds), (vaX1, vaX2, vaMem, vaIds)], open(data_dir + '/' + 'c' * args.comet + 'h' * (1-args.comet) + '_' + args.use_filter * 'filtered_' +  args.kg_type + '_' + 'databart.pkl','wb'))
            print("Done....")
