...
if __name__ == '__main__':
    ...
    def clean_gen(gen):
        gen = [w for w in gen.tolist() if w != text_encoder.pad_token_id]
    ...
    def pad_rels(relation, pad_len=100):
        return relation[:100] + [text_encoder.pad_token_id] * (100-len(relation[:100]))
            ...
            for sent_id in tqdm(sent_ids):
                ...
                with torch.no_grad():
                     for d in tqdm(range(len(dims))):
                         XMB = [text_encoder.pad_token_id] * 256
                         ...
                         hint = (text_encoder.convert_tokens_to_ids(
                             text_encoder.tokenize(" hint: "+line_["hint"])) if "hint" in line_ else [])

                         context =text_encoder.encode("source: ") +context+ [sent_id, dims[d]] +hint+ [text_encoder.eos_token_id]

                         save_output["hint"] = (line_["hint"]) if "hint" in line_ else False
                         # print("Input",text_encoder.decode(context))
                         i_1 = len(context)-1
