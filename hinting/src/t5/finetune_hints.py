...
def main():
    ...
    completed_steps = 0
    if args.do_train:
        for epoch in trange(args.num_train_epochs,desc="Epoch",disable=not accelerator.is_local_main_process):
            model.train()
            if args.hints:
                use_hints = True
            else:
                use_hints = False
            for step, batch in enumerate(tqdm(train_dataloader,desc="Iteration",disable=not accelerator.is_local_main_process)):
                outputs = model(**batch)
