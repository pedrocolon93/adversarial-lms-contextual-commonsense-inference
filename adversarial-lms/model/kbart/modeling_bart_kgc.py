...
try:
    from fairscale.nn import wrap, auto_wrap, checkpoint_wrapper, enable_wrap, default_auto_wrap_policy
except:
    print("Could not load fairscale!!!")
...
from transformers.modeling_outputs import BaseModelOutput, BaseModelOutputWithPastAndCrossAttentions, \
    Seq2SeqModelOutput, Seq2SeqLMOutput, Seq2SeqSequenceClassifierOutput, Seq2SeqQuestionAnsweringModelOutput, \
...
class BartDecoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # Use memory for Knowledge Retention
        self.encoder_memory = False
        self.decoder_memory = True
...
def shift_embeddings_right(inputs_embeds, decoder_start_token_vec):
    shifted_input_ids = inputs_embeds.new_zeros(inputs_embeds.shape)
    shifted_input_ids[:, 1:, :] = inputs_embeds[:, :-1, :]  # .clone()
    shifted_input_ids[:, 0, :] = decoder_start_token_vec
    return shifted_input_ids
    ...
    def forward(
        ...
        if decoder_input_ids is None and decoder_inputs_embeds is None:
            if input_ids is not None:
                ...
                decoder_inputs_embeds = shift_embeddings_right(
                    inputs_embeds, self.shared(
                        torch.tensor([self.config.decoder_start_token_id]).to(self.shared.weight.device)).unsqueeze(0)
                )
...
class BartForSequenceClassification(BartPretrainedModel):
    ...
    def forward(
        ...
        # if input_ids is None and inputs_embeds is not None:
            # encoder_outputs = self.model.encoder(inputs_embeds=inputs_embeds, return_dict=True,
        ...
        if noise:
            noise = torch.randn(sentence_representation.shape).to(sentence_representation.device)
            # mn = torch.mean(sentence_representation)
            sentence_representation+= noise + 0.1*torch.ones(noise.shape,device=noise.device)
...
class KBartForConditionalGeneration(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]
    ...
    def __init__(self, config: BartConfig):
        ...
        self.r_embed = nn.Embedding(config.vocab_size, config.d_model)
        self.mem_len = 128
        self.proj1 = nn.Linear(self.mem_len, 1)
        self.config = config
        self.diff = torch.nn.CosineSimilarity()
        self.mem = torch.tensor([[]])
        # self.sargmax = self.SoftArgmax1D()
        self.init_weights()

    def resize_memory_embeddings(self,new_length):
        self.r_embed = nn.Embedding(self.config.vocab_size, self.config.d_model)
    ...
    def update_mem(self,update_mem):
        update_mem = self.r_embed(torch.tensor(update_mem))
        if self.mem.shape[1]==0:
            self.mem = update_mem
        else:
    ...
    def forward(
        ...
        if clear_mem == True:
            self.mem = torch.zeros((hidden_states.size(0),0,self.mem_len,self.config.d_model)).to(self.device)#.cuda()
        self.mem = self.mem.type(hidden_states.type())

        if update_mem is not None:
            self.update_mem(update_mem)

        if use_mem and size_mem != 0 and self.mem.size(1)!=0:
            mean_mems = torch.mean(self.mem, dim=2)
            mean_context = torch.mean(hidden_states, dim=1)
            if mean_mems.shape[0]!=mean_context.shape[0]:
                self.mem = self.mem.repeat(int(mean_context.shape[0]/mean_mems.shape[0]),1,1,1)
                mean_mems = mean_mems.repeat(int(mean_context.shape[0]/mean_mems.shape[0]),1,1)
            ...
            retrieve = retrieve.type(hidden_states.type())
